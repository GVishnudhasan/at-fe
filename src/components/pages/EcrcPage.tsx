...
import EcrcTemplate from '@/components/templates/EcrcTemplate';
...
const defaultTableData = tableWiseDefaultData?.projectECRCDefaultData;
...
const EcrcPage = () => {
  ...
  } = useDataTableLogics(defaultTableData, tableRelations?.projectECRCPage);
  ...
  return (
    <ModalWrapper
      ...
      columnWiseListForFilter={columnWiseListForFilter?.projectECRCDefaultData(
        tableHeader
      )}
      uploadFormInputsWithLabelAndKey={uploadFormInputsWithLabelAndKey?.projectECRCDefaultData(
        tableHeader
      )}
    ...
    >
      <EcrcTemplate {...(passAsProps as any)} />
    </ModalWrapper>
  );
};

export default EcrcPage;
