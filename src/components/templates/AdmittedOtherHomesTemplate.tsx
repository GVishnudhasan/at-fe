...
import { admittedInOtherHomesDataKeys } from '@/utils/tableDataUtils';
import type { TemplateType } from '@/utils/typeUtils';

type AdmittedOtherHomesDataType = {
  id: string;
  otRegNo: string;
  outreachDate: Date;
  auin: string;
...
interface AdmittedOtherHomesType {
  [name: string]: AdmittedOtherHomesDataType;
}

export type ReqDatType = AdmittedOtherHomesType[];

const AdmittedOtherHomesTemplate = ({
  appliedFilter = [],
  tableData,
  totalCount,
  selectedData,
  ...
  return (
    <div className="mx-auto mt-12 h-full w-[85%] md:w-[95%]">
      <PageHeaderSection
        pageTitle="Admitted in other homes"
      ...
      {!loading?.tableLoading || true ? (
        <TableSection
          ...
          tableDataKeys={admittedInOtherHomesDataKeys}
...
export default AdmittedOtherHomesTemplate;
